...
select
    substring('abcdefghijkl' from 1 for 3);

/* Select the first three characters of the string */
--but have an alias for the column name instead of substring
select
...
-- it's including the space
select
   job_title, substring(job_title from 11) asst_category
from
   staff
where
   job_title like 'Assistant%';
...
/* Give the position where Assistant appears in job_title */
select
